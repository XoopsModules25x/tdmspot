...
$gperm_handler =& xoops_gethandler('groupperm');
	...
	$select_cat = '<select name="form_cat" onchange="window.document.location=this.options[this.selectedIndex].value;">';
	
	$select_cat .= '<option >-- '._AM_SPOT_CATEGORY.'</option>';
	
	foreach (array_keys($cat_arr) as $i) {
	$select =  ($cat == $cat_arr[$i]->getVar("id")) ? 'selected="selected"' : false;
	$pid =  ($cat_arr[$i]->getVar("pid") != 0) ? '--' : false;
	if ($gperm_handler->checkRight('tdmspot_catview', $cat_arr[$i]->getVar('id'), $groups, $xoopsModule->getVar('mid'))) {
	$cat_link = tdmspot_seo_genUrl( $xoopsModuleConfig['tdmspot_seo_cat'], $cat_arr[$i]->getVar('id'), $cat_arr[$i]->getVar('title'), $start, $limit, $tris );
	$select_cat .= '<option '.$select.' value="'.$cat_link.'"> '.$pid.$cat_arr[$i]->getVar("title").' </option>';
	$i++;
	}
	}
	$select_cat .= '</select>';
...
$gperm_handler =& xoops_gethandler('groupperm');
	...
	$page_arr = $page_handler->getall();
	$select_page = '<select name="form_cat" onchange="window.document.location=this.options[this.selectedIndex].value;">';
	
	$select_page .= '<option >- '._AM_SPOT_PAGE.'</option>';
	
	foreach (array_keys($page_arr) as $i) {
	$select =  ($page == $page_arr[$i]->getVar("id")) ? 'selected="selected"' : false;
	if ($gperm_handler->checkRight('spot_pageview', $page_arr[$i]->getVar('id'), $groups, $xoopsModule->getVar('mid'))) {
	$cat_link = tdmspot_seo_genUrl( 'index', $page_arr[$i]->getVar('id'), $page_arr[$i]->getVar('title') );
	$select_page .= '<option '.$select.' value="'.$cat_link.'"> '.$page_arr[$i]->getVar("title").' </option>';
	$i++;
	}
	}
	$select_page .= '</select>';
